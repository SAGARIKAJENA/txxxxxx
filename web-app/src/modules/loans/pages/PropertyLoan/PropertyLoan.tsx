...
import { propertyLoanValidation } from './validation/propertyLoanValidation'
...
const INITIAL_PROPERTY_LOAN_DATA: PropertyLoanData = {
  ...
  estimatedMarketValue: '1,20,00,000',
  propertyLocationCity: 'Koramangala, Bengaluru',
  repaymentTenureYears: 15,
  primaryIncomeSource: 'Business Owner',
  monthlyHouseholdIncome: '₹2,50,000 - ₹5,00,000',
  hasExistingPropertyLoan: false,
  operatingBank: 'State Bank of India (SBI)',
...
export const PropertyLoan: React.FC = () => {
  ...
  const {
    ...
    saveDraft,
    discardDraft,
  } = useLoanApplication<PropertyLoanData>('property_loan', INITIAL_PROPERTY_LOAN_DATA)

  const validateCurrentStep = (): boolean => {
    setStepError(null)
    if (currentStep === 1) {
      const res = propertyLoanValidation.validateStep1(formData)
      if (!res.isValid) {
        setStepError(res.error || 'Please fill required fields in Step 1.')
        return false
      }
    } else if (currentStep === 2) {
      const res = propertyLoanValidation.validateStep2(formData)
      if (!res.isValid) {
        setStepError(res.error || 'Please fill required fields in Step 2.')
        return false
      }
    } else if (currentStep === 3) {
      const res = propertyLoanValidation.validateStep3(formData)
      if (!res.isValid) {
  ...
  return (
    <div className="property-loan-page">
      <LoanPageNavigation title="Property Loan" />
