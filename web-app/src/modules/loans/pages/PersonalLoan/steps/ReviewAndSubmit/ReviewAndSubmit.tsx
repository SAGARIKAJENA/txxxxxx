import React from 'react'
import { LoanReviewSection } from '../../../../components/LoanReviewSection/LoanReviewSection'
import type { PersonalLoanData } from '../../types/personalLoan.types'
import './ReviewAndSubmit.css'

export interface ReviewAndSubmitProps {
  formData: PersonalLoanData
  updateFormData: (fields: Partial<PersonalLoanData>) => void
  onNavigateToStep: (stepNumber: number) => void
}

export const ReviewAndSubmit: React.FC<ReviewAndSubmitProps> = ({
  formData,
  updateFormData,
  onNavigateToStep,
}) => {
  const docCount = Object.keys(formData.uploadedDocs || {}).length

  return (
    <div className="personal-loan-review">
      {/* 1. Requirements */}
      <LoanReviewSection
        title="Personal Loan Requirements"
        onEdit={() => onNavigateToStep(1)}
        icon={
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <line x1="12" y1="1" x2="12" y2="23" />
            <path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6" />
          </svg>
        }
        items={[
          {
            label: 'Requested Loan Amount',
            value: `₹${(formData.loanAmount || 0).toLocaleString('en-IN')}`,
          },
          {
            label: 'Loan Purpose',
            value: formData.loanPurpose || 'Not specified',
          },
          {
            label: 'Repayment Tenure',
            value: `${formData.repaymentTenureYears} Years (${formData.repaymentTenureYears * 12} Months)`,
          },
        ]}
      />

      {/* 2. Employment & Income */}
      <LoanReviewSection
        title="Employment & Income Profile"
        onEdit={() => onNavigateToStep(2)}
        icon={
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <rect width="20" height="14" x="2" y="7" rx="2" />
            <path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16" />
          </svg>
        }
        items={[
          {
            label: 'Occupation Type',
            value: (formData.occupation || '').replace(/-/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase()),
          },
          {
            label: 'Employer / Company',
            value: formData.employerName || '—',
          },
          {
            label: 'Monthly Take-Home',
            value: formData.monthlyTakeHomeSalary || '—',
          },
          {
            label: 'Ongoing EMIs',
            value: formData.hasExistingEmis
              ? `₹${formData.existingEmiAmount || '0'} / month`
              : 'None',
          },
        ]}
      />

      {/* 3. Banking & Tax Details */}
      <LoanReviewSection
        title="Banking & Tax Identity"
        onEdit={() => onNavigateToStep(3)}
        icon={
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <rect width="20" height="14" x="2" y="5" rx="2" />
            <line x1="2" y1="10" x2="22" y2="10" />
          </svg>
        }
        items={[
          {
            label: 'Salary Bank',
            value: formData.salaryBankName || '—',
          },
          {
            label: 'Account Number',
            value: formData.accountNumber ? `••••${formData.accountNumber.slice(-4)}` : '—',
          },
          {
            label: 'IFSC Code',
            value: formData.ifscCode || '—',
          },
          {
            label: 'PAN Number',
            value: formData.panNumber || '—',
          },
        ]}
      />

      {/* 4. Uploaded Documents */}
      <LoanReviewSection
        title={`Uploaded Documents (${docCount})`}
        onEdit={() => onNavigateToStep(4)}
        icon={
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z" />
            <polyline points="14 2 14 8 20 8" />
          </svg>
        }
        items={[
          {
            label: 'Documents Attached',
            value: docCount > 0 ? `${docCount} documents attached` : 'No documents uploaded',
          },
        ]}
      />

      {/* Declaration */}
      <div className="personal-loan-review__declaration">
        <input
          id="personal-loan-terms-checkbox"
          type="checkbox"
          className="personal-loan-review__checkbox"
          checked={formData.termsAccepted}
          onChange={(e) => updateFormData({ termsAccepted: e.target.checked })}
        />
        <label htmlFor="personal-loan-terms-checkbox" className="personal-loan-review__label">
          I confirm that all provided details and attached documents are accurate and authentic to the best of my knowledge, and I accept the{' '}
          <span className="personal-loan-review__terms-link">Terms &amp; Conditions</span> of the Personal Loan application.
        </label>
      </div>
    </div>
  )
}

export default ReviewAndSubmit
